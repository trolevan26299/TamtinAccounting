...
let InvoicesService = class InvoicesService {
    ...
    formatGDTDate(date, endOfDay = false) {
    ...
    async fetchPage(gdtToken, searchQuery, loaiHD, state, size = PAGE_SIZE, retries = 3) {
        ...
        if (lastError) {
            if (lastError.code === 'ECONNABORTED' || lastError.message?.includes('timeout')) {
                return { status: 504, data: { message: `Kết nối tới GDT server bị quá hạn (Timeout) sau ${retries} lần thử. Hệ thống Thuế hiện đang quá tải nặng.` } };
            }
            return { status: 502, data: { message: 'Không thể kết nối đến GDT: ' + lastError.message } };
        }
        return { status: 502, data: { message: `Đã thử chạy lại ${retries} lần nhưng GDT vẫn báo lỗi quá tải.` } };
    }
    async fetchAllPages(gdtToken, searchQuery, loaiHD) {
        const all = [];
        let state = null;
        let total = 0;
        do {
            ...
            if (res.status !== 200) {
                ...
                throw new common_1.BadGatewayException(`GDT lỗi ${res.status}: ${errorDetail}`);
            }
    ...
    async search(gdtToken, fromDate, toDate, loaiHD, state) {
        ...
        if (res.status !== 200) {
            ...
            throw new common_1.BadGatewayException(errorDetail);
        }
        return {
    ...
    async exportExcel(gdtToken, fromDate, toDate, loaiHD, mst, res) {
        const from = new Date(fromDate);
        const to = new Date(toDate);
        ...
        sheet.getCell('B7').value = 'Hóa đơn chứng từ bán';
        sheet.getCell('B8').value = 'Seri';
        sheet.getCell('C8').value = 'Số HĐ';
        sheet.getCell('D8').value = 'Ngày HĐ';
        sheet.mergeCells('E7:E8');
        sheet.getCell('E7').value = 'Tên người mua';
        sheet.mergeCells('F7:F8');
        sheet.getCell('F7').value = 'Mã số thuế';
        sheet.mergeCells('G7:G8');
        sheet.getCell('G7').value = 'Mặt hàng';
        sheet.mergeCells('H7:H8');
        sheet.getCell('H7').value = 'Doanh số bán\nchưa thuế';
        sheet.mergeCells('I7:I8');
        sheet.getCell('I7').value = 'Thuế\nsuất';
        sheet.mergeCells('J7:J8');
        sheet.getCell('J7').value = 'Thuế\nGTGT';
        sheet.mergeCells('K7:K8');
        sheet.getCell('K7').value = 'Ghi chú';
        const borderThin = {
    ...
    async fetchPurchasePage(gdtToken, searchQuery, from, loaiHD = 'ddt', size = PAGE_SIZE, retries = 3) {
        const isMtt = loaiHD === 'mtt';
        ...
        if (lastError)
            return { status: 502, data: { message: 'Không thể kết nối GDT (mua vào): ' + lastError.message } };
        return { status: 502, data: { message: `Đã thử ${retries} lần nhưng GDT mua vào vẫn lỗi.` } };
    }
    async searchPurchase(gdtToken, fromDate, toDate, loaiHD = 'ddt', pageOffset = 0) {
        const from = new Date(fromDate);
        const to = new Date(toDate);
        ...
        if (res.status === 401)
            return { gdtExpired: true, error: 'Token GDT đã hết hạn' };
        if (res.status === 204 || res.status === 404)
            return { datas: [], total: 0, offset: pageOffset };
        if (res.status !== 200) {
    ...
    async exportPurchaseExcel(gdtToken, fromDate, toDate, loaiHD = 'ddt', mst, res) {
        const from = new Date(fromDate);
        const to = new Date(toDate);
        const diffDays = Math.ceil((to.getTime() - from.getTime()) / (1000 * 60 * 60 * 24)) + 1;
        const allData = [];
        ...
        for (const chunk of chunks) {
            ...
            do {
                ...
                if (pageRes.status !== 200) {
                    ...
                    throw new common_1.BadGatewayException(`Lỗi GDT (mua vào): ${errorDetail}`);
                }
        ...
        sheet.getCell('A7').value = 'STT';
        sheet.mergeCells('B7:D7');
        sheet.getCell('B7').value = 'Hóa đơn chứng từ mua';
        sheet.getCell('B8').value = 'Seri';
        sheet.getCell('C8').value = 'Số HĐ';
        sheet.getCell('D8').value = 'Ngày HĐ';
        sheet.mergeCells('E7:E8');
        sheet.getCell('E7').value = 'Tên người bán';
        sheet.mergeCells('F7:F8');
        sheet.getCell('F7').value = 'MST người bán';
        sheet.mergeCells('G7:G8');
        sheet.getCell('G7').value = 'Mặt hàng';
        sheet.mergeCells('H7:H8');
        sheet.getCell('H7').value = 'Giá trị hàng hóa\nchưa thuế';
        sheet.mergeCells('I7:I8');
        sheet.getCell('I7').value = 'Thuế\nsuất';
        sheet.mergeCells('J7:J8');
        sheet.getCell('J7').value = 'Thuế\nGTGT';
